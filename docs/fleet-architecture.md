...
| 2 | Game streaming moves: one-way copy of the ROM library (production stays master because RomM reads it); bring up streaming on the new GPU | A real game plays end to end |
...
8. **NAS export for the new machine (2026-09-20):** the NAS's UGOS web UI has no page for per-host NFS
   ...
   **Stale since 2026-09-21:** r9 is now `192.168.0.22`. The NAS export now lists `.22` (ro, added 2026-09-21; backup `/etc/exports.bak.pre-r9-ip22`). Test mount from r9 verified (NFSv3 like production; NFSv4 is not offered). Remaining: remove the stale `.149` entry. Flip `.22` to `rw` at cutover.
