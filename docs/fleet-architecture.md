...
| 4 | **COMPLETE 2026-09-21 (closure proposed by Claude, Jay to confirm).** **Amended by D3:** only the compute-role services move to r9 (Immich, Jellyfin, Minecraft, Stirling PDF and similar); the essentials and the *arr pipeline stay on production. One stack at a time, each within F1's seconds-to-minutes | Each stack verified before the next Outcome: Minecraft (3 servers) moved to r9 (5.4); Stirling PDF moved behind r9's own Caddy (D7, D8); Jellyfin and RomM retired (D9); Immich's machine learning moved to r9's GPU with server, database and library left on production (D10); Immich originals put on two devices (D11). New platform pieces: r9's front door with its own name-constrained CA intermediate (D6); Wings node on r9. Production went from 52 to 49 running containers and swap in use fell from about 3,450 MB to about 2,600 MB. |
| 5 | Old production machine repurposed (storage/backup role, proposed) | Library safely served or moved (option C) first |
| 6 | **ON HOLD 2026-09-24 (Jay): no cameras and none expected for a while; migration cleanup takes priority. Software shell on x51 stays up.** Started 2026-09-22. Security system on its own machine (F4): x51's clock (dead RTC battery, stuck ~2 months behind, silently broke Tailscale login), ufw and Tailscale rejoin fixed first; Home Assistant, Frigate and a Mosquitto broker deployed. Frigate's GPU passthrough to the GTX 1650 is verified but running the CPU detector as a placeholder — no cameras exist yet, so there's nothing to detect against | Independent of phases 1–5. **Not yet gated closed:** no cameras chosen/bought; ONNX/TensorRT detector not enabled (needs a one-time model export, deferred to do alongside real camera setup); footage-on-NAS still waits on the off-site backup being fully finished (all 8 sources are off-site as of 2026-09-24, but the scheduled run is held until a ~2026-10-02 Google token durability check) |
...
**Update 2026-09-24: option D adopted, with 200 (not 100).** Production AdGuard restarted (x51 covered it) with `ratelimit: 200` and `ratelimit_subnet_len_ipv4: 32`; x51 got the same through the hourly sync. Why 200: in the query log (peaks are a floor, since the old limit of 20 hid them before 9/21) a limit of 100 would have clipped five devices including phones (127-186 q/s bursts, 42 device-seconds); 150 clipped three; 200 clipped only the gaming PC's retry loops (9 device-seconds); 300 clipped none. Verified: one device (r9) bursting 600 queries got 198-200 answered on both AdGuards, and a second device's lookups during that burst answered 30 of 30. Live-file backup: `/srv/docker/adguardhome/conf/AdGuardHome.yaml.pre-ratelimit-20260924`. Rollback is now: set `ratelimit: 500`, `ratelimit_subnet_len_ipv4: 24` (AdGuard stopped while editing), restart; the sync carries it to x51.
...
4. **Device acceptance verified (3 agreeing sources):** Jay added the AdGuard rewrite `r9test.lan` to `100.121.244.45` (format identical to the other entries; 37 to 38 rewrites) and opened `https://r9test.lan:18443/` on the iPhone (iOS 18.7, Safari) and the Windows gaming PC (Chrome/Edge): the page loaded with no certificate warning on either (browsers no longer show a padlock; the absence of a warning page is the signal). r9's Caddy access log independently recorded both devices completing TLS 1.3 with SNI `r9test.lan`, status 200, and had 0 handshake errors. **Negative test verified on devices (2026-09-21):** with a second temporary rewrite (`constraint-test.example.com` to `100.121.244.45`), both the iPhone (Safari) and the Windows gaming PC (Brave, Chromium-based; earlier notes said Chrome/Edge in error) showed a certificate warning, and r9's Caddy debug log recorded each client refusing the handshake (`remote error: tls: unknown certificate` from `100.97.193.57` five times and `100.115.217.47`), with no accepted request for that name. So Apple's and Chromium's verifiers **enforce** the `.lan`-only name constraint. **Not tested:** Firefox, Android, Edge, the TV. **Lesson recorded:** Caddy logs handshake failures at debug level only, so 'zero handshake errors' at default logging proves nothing; enable debug when testing rejections. Remaining: remove the temporary rewrites (`constraint-test.example.com` now; `r9test.lan` after the 443 move). Name-constraint enforcement is proven for OpenSSL/curl only; enforcement on iOS, Windows and Android is expected but not verified.
...
**Failure path tested (2026-09-21 18:04, announced and approved by Jay):** the script run with an empty source exited 1, logged `FAILED: source has only 0 files (floor 20000)`, and Kuma recorded status 0 (down, `failed:_mirror`, important=1); a normal run then exited 0 and Kuma recorded status 1 (up, important=1). **Jay confirmed both Discord messages arrived** (18:05:02 'went down' with `failed:_mirror`, 18:06:42 'is up'), so the whole chain (script, Kuma, Discord) is verified end to end. The two test runs each left an extra database dump (17 kept; the 14-day retention prunes them). **Not verified:** photo browsing speed from the NAS in daily use (thumbnails were not moved); behavior when `/mnt/nas` itself is unmounted at 02:30 (guarded by `mountpoint -q` in the script, not exercised).
...
| Q9 | Second AdGuard on the x51 as DNS failover? | **CLOSED 2026-09-24 (Jay confirmed the Kuma test alerts reached Discord).** See the 2026-09-24 updates below. Original question: AdGuard holds 37 `.lan` rewrites that public DNS cannot answer; production reboots automatically Wed and Sun at 03:00, so `.lan` names and ad-blocking drop for a few minutes each time. Moderate value, low urgency; needs the two configs kept in sync. **Update 2026-09-21:** r9's resolver is sticky (never returns to the first-listed server after a failure), so any AdGuard outage (production's Wed/Sun 03:00 reboot) leaves r9 on `1.1.1.1` with no `.lan` names. A stopgap timer on r9 (`scripts/r9-resolved-prefer-adguard/`, every 2 min) returns it to AdGuard once AdGuard answers. With a second AdGuard, drifting to the second one would be harmless, which is the structural fix. **Update 2026-09-24 (built, clients NOT yet pointed at it):** replica AdGuard runs on x51 (`stacks/adguard-x51/`, pinned to production's v0.107.72). `scripts/adguard-sync-x51.py` copies production's config one-way (rewrites, blocklists, rules, clients, rate limit), overriding only the admin user and the upstreams, which are plain UDP (`9.9.9.10`, `1.1.1.1`, DoH fallback) so a DoH/TCP stall on production's path (the 2026-09-20 outage, `docs/dns-resilience.md`) does not hit both. All 38 rewrites verified identical from both. **Client change rule:** ADD x51 to client lists, never replace `1.1.1.1` (router DHCP `.21, .20, 1.1.1.1` only if the SBG8300 takes three entries; Tailscale `lan` split route + global list between production and `1.1.1.1`). **Update 2026-09-24 (later):** hourly sync cron installed; failover/failback test passed on r9; Tailscale admin console done by Jay and verified via API (global: production, x51, Cloudflare preset; `lan` split route: production, x51). **Router decision (2026-09-24, Jay):** SBG8300 (Gateway > Connection > Local IP Network > IPv4) has only Primary/Secondary DNS fields (`.21`, `1.1.1.1`), so DHCP is deliberately LEFT UNCHANGED: x51 in the second slot would trade protection against a whole-site outage (power loss, both machines rebooting) for a 3 AM ad-block gap. Tailscale devices carry all three resolvers. **Client test passed** (jays-iphone over Tailscale, production AdGuard stopped 60 s: all lookups answered by x51, `.lan` + ad-blocking intact; see `docs/dns-resilience.md`). **Sync alerting:** Kuma push monitor #44 "AdGuard x51 sync" (2 h window, Discord); the script reports up/down on every run and verifies x51 answers a `.lan` name. DB backup before the insert: `/srv/docker/visibility/uptime-kuma/kuma.db.pre-adguard-x51-monitor`. Jay confirmed the deliberate down/up test reached Discord. |
...
| 2026-09-21 | Phase 4 complete: compute-role services moved or retired (D6-D11); see section 5f-5k and the Phase 4 row. Open follow-ups are in the private backlog (off-site photo copy, single-disk NAS, WUD blind to r9, x51 firewall, 30-day cleanup of retired services). |
