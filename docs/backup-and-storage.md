...
- **The OAuth app is in Testing mode, so its refresh token expires after 7 days** (this one issued 2026-09-23, dies about 2026-09-30). Publishing to "In production" is blocked: the Audience page says "To publish your app, you must complete your configuration on the Branding page" without naming the field. Per Google's Manage OAuth App Branding help page, the homepage, privacy policy and terms-of-service links are required for ALL external production apps, and a Google developer-forum thread reports the same vague error clearing only once homepage + privacy policy URLs (and then Authorized domains) were filled in. Jay's Branding page has all of those empty, so that is the likely blocker (the developer-contact-email theory recorded here earlier was a wrong guess). Needs a public page on a domain that can be an Authorized domain -- still to be decided. After publishing, re-run `rclone authorize` for a fresh token and revoke the old grant at myaccount.google.com/permissions.
